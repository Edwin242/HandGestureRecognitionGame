...
export class Physics {
  ...
  update(dt, player, world) {
    ...
    while (this.accumulator >= this.stepSize) {
      ...
      if (player.isJumping && player.onGround){
        ...
        player.camera.getWorldDirection(forwardDirection);
        ...
        const jumpForwardSpeed = 2;
        player.velocity.x += forwardDirection.x * jumpForwardSpeed;
        player.velocity.z += forwardDirection.z * jumpForwardSpeed;
  ...
  resolveCollisions(collisions, player) {
    ...
    for (const collision of collisions) {
      ...
      if (Math.abs(collision.normal.y) < 0.5 && player.onGround){
        this.applyCollisionJump(player);
      }

      if (collision.normal.y > 0.5) {
        player.onGround = true;
        player.isJumping = false; // Reset isJumping flag
        player.velocity.y = 0;
      ...
      if (Math.abs(collision.normal.y) < 0.5 && player.onGround && !player.isJumping) {
        this.applyCollisionJump(player);
      }
  ...
  applyCollisionJump(player){
    if(player.isCollisionJumping) return;
    player.isCollisionJumping = true;

    player.velocity.y = 10;

    const forwardDirection = player.getForwardVector();
    forwardDirection.y = 0;
    forwardDirection.normalize();

    const collisionJumpForwardSpeed = 5;
    player.velocity.x += forwardDirection.x * collisionJumpForwardSpeed;
    player.velocity.z += forwardDirection.z * collisionJumpForwardSpeed;

    player.onGround = false;

    setTimeout(() => {
      player.isCollisionJumping = false;
    }, 500);
